...
def returnSmoothGrad(img, model, criterion, num_samples=50, sigma=0.2, device='cpu'):
    model.to(device)
    img = img.to(device)
    img.requires_grad_(True).retain_grad()
...
if __name__ == '__main__':
    ...
    smooth_gradient = returnSmoothGrad(img=img, model=net, criterion=criterion)
